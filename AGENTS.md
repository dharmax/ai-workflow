...
If the ai-workflow tool is unavailable, follow the same process with `aiwf-shell/scripts/ai-workflow/*`.
...
Default context order: aiwf MCP/DB graph, targeted `ai-workflow` CLI extraction, `lean-ctx` search/read/tree, then broad raw file reads only when the narrower surfaces cannot answer.
