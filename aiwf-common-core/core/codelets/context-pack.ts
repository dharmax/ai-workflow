...
import { getToolkitRoot, getWorkspaceRoot } from "../lib/toolkit-root.ts";
...
${ticket.body}` : "";
  ...
  const [agents, contributing, executionProtocol, enforcement, guidelines, manual, knowledge] = await Promise.all([
    ...
    readText(path.resolve(root, "docs", "MANUAL.md"), await readText(path.resolve(getWorkspaceRoot(toolkitRoot), "docs", "MANUAL.md"))),
