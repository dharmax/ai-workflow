...
import { renderManualHtml } from "./manual-html.ts";
import {
...
for (const [surfaceId, snapshot] of Object.entries(operatorSurfaces)) {
  ...
  if (!dogfoodReport) {
    findings.push(createFinding({
      ...
      message: `missing dogfood report for operator surface -> ${surfaceId}. Run tsx aiwf-shell/scripts/ai-workflow/dogfood.mjs`
    }));
    continue;
  }
  ...
  if (!reportedSurface) {
    findings.push(createFinding({
      category: "dogfood",
      file: DEFAULT_DOGFOOD_REPORT_PATH,
      message: `dogfood report missing operator surface -> ${surfaceId}. Run tsx aiwf-shell/scripts/ai-workflow/dogfood.mjs --surface ${surfaceId}`
    }));
    continue;
  }
  ...
  if (!compareSurfaceHashes(reportedSurface, snapshot)) {
    findings.push(createFinding({
      category: "dogfood",
      file: DEFAULT_DOGFOOD_REPORT_PATH,
      message: `dogfood report is stale for operator surface -> ${surfaceId}. Re-run tsx aiwf-shell/scripts/ai-workflow/dogfood.mjs`
    }));
  }
