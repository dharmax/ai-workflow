...
import { extractMarkdownCandidates, tokenize } from "./markdown-utils.ts";
...
export async function loadProjectActiveGuardrails(root, options = {}) {
  ...
  const manualFallback = await readText(path.resolve(getWorkspaceRoot(toolkitRoot), "docs", "MANUAL.md"));
