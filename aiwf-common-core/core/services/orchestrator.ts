 ...
 * @brief Auto-generated header for orchestrator.js. Needs detailed responsibility and scope.
 */

import path from "node:path";
import { readFile } from "node:fs/promises";
import { parsePatch, applyPatch } from "@dharmax/block-patcher";
import { buildSurgicalContext, formatContextForPrompt } from "./context-packer.ts";
import { readProjectFile, writeProjectFile } from "../lib/filesystem.ts";
import { routeTask } from "./router.ts";
import { generateCompletion } from "./providers.ts";
import { withWorkflowStore } from "./sync.ts";
import { buildTicketEntity, writeProjectProjections } from "./projections.ts";
