/**
 * @file knowledge.js
 * @brief Auto-generated header for knowledge.js. Needs detailed responsibility and scope.
 */
...
export async function recordProjectKnowledge({
  ...
  const normalizedLessons = normalizeStringArray(lessons).slice(0, 5);
  for (const lesson of normalizedLessons) {
    lines.push(`  - ${lesson}`);
  }
  if (Array.isArray(changedFiles) && changedFiles.length) {
    const changedFilesLine = `Changed files: ${changedFiles.join(", ")}`;
    if (!normalizedLessons.some((lesson) => lesson.includes(changedFilesLine))) {
