/**
 * @file codelets.js
 * @brief Auto-generated header for codelets.js. Needs detailed responsibility and scope.
 */
...
export { getToolkitRoot } from "../lib/toolkit-root.ts";

const codeletRegistryCache = new Map();

export function getSharedCodeletsDir(toolkitRoot = getToolkitRoot()) {
  return path.resolve(getCommonCoreRoot(toolkitRoot), "shared", "codelets");
}
...
function materializeCodeletRecord(entity) {
  ...
  return {
    ...
    toolPolicy: data.toolPolicy ?? null,
    graderId: data.graderId ?? null,
    maxRetries: Number.isFinite(Number(data.maxRetries)) ? Number(data.maxRetries) : 1,
