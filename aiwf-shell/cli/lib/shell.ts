...
import { ExecutionMode, detectExecutionMode } from "aiwf-common-core/services/execution-context";
...
import { discoverProviderState, generateCompletion, summarizeCompletionUsage } from "aiwf-common-core/services/providers";
import { decomposeTicket, executeTicket, ideateFeature, ingestArtifact, sweepBugs } from "aiwf-common-core/services/orchestrator";
import { auditArchitecture } from "aiwf-common-core/services/critic";
import { addManualNote, createTicket, evaluateProjectReadiness, getCodelet, getProjectMetrics, getProjectSummary, getSmartProjectStatus, recordMetric, searchProject, syncProject, updateTicketLifecycle, withWorkflowStore } from "aiwf-common-core/services/sync";
import { executeCodelet } from "aiwf-common-core/services/codelet-executor";
import { buildTicketEntity, inferTicketLane } from "aiwf-common-core/services/projections";
import { buildTelegramPreview } from "aiwf-common-core/services/telegram";
import { parseArgs, printAndExit } from "aiwf-common-core/lib/cli";
import { loadProjectActiveGuardrails, selectActiveGuardrails } from "aiwf-common-core/lib/active-guardrails";
...
import { runProviderSetupWizard } from "./provider-setup.ts";
import { handleProviderConnect } from "./provider-connect.ts";
import { stableId } from "aiwf-common-core/lib/hash";
import { withWorkspaceMutation } from "aiwf-common-core/lib/workspace-mutation";
import { collectProjectFiles, readProjectFile, writeProjectFile, loadPromptTemplate, renderTemplate } from "aiwf-common-core/lib/filesystem";
import { formatStatusReport, resolveProjectStatus } from "aiwf-common-core/services/status";
import { getRelevantGuidelineBlocks } from "aiwf-common-core/services/guidelines";
...
export async function planShellRequest(inputText, options) {
  ...
  if (triage.intent.tier === ShellTier.Tier1) {
    if (options.noAi) {
      const strictInput = triage.intent.extracted?.primitive ?? inputText;
      return normalizeShellPlanEnvelope(
        buildStrictNoAiShellPlan(strictInput, options.plannerContext, options),
        inputText,
...
async function buildGroundedShellFallbackPlan(inputText, options) {
  ...
  for (const selector of selectors.slice(0, 4)) {
    const payload = await ServiceHub.execute("status", {
      projectRoot,
      selector,
      includeRelated: true,
      rawQuestion: true,
...
export function planShellRequestHeuristically(inputText, plannerContext, options = {}) {
  ...
  if (followUpMode !== "new-request" && activeGraphState?.graph?.nodes?.length) {
    ...
    const hasContinuationAnchor = Boolean(
      ...
      || activeGraphState?.focus?.searchQuery
      || activeGraphState?.references?.files?.length
      || activeGraphState?.references?.modules?.length
    );
...
function buildStrictNoAiShellPlan(inputText, plannerContext = {}, options = {}) {
  ...
  const normalized = normalizeConversationText(text);
  const readinessGoal = extractReadinessGoal(text);
  const wantsCombinedStatusReadiness = Boolean(
    readinessGoal
    && (normalized === "project summary"
      || normalized === "summary"
      || normalized === "status"
      || /\b(status|project)\b/.test(normalized))
  );
  ...
  if (/\b(resolve|finalize)\b/.test(normalized) && /\bin[ -]?progress\b/.test(normalized)) {
    return replyPlan(renderStrictNoAiReply(), 0.98, "No-AI mode requires an explicit ticket id for in-progress mutations.");
  }

  if (normalized === "what project am i in and what should i work on next"
    || normalized === "which classes do we have in the project"
    || normalized === "so whats up") {
    return replyPlan(renderStrictNoAiReply(), 0.98, "No-AI mode rejected an ungrounded broad repo question.");
  }
  ...
  if (wantsCombinedStatusReadiness) {
    return {
      ...
      confidence: 1,
      reason: "No-AI mode routed the request to local project summary plus readiness evaluation.",
      planner: { mode: "heuristic-forced", reason: "no-ai-status-readiness" },
      presentation: "assistant-first"
    };
  }
  ...
  if (normalized === "project summary" || normalized === "summary" || normalized === "status" || /\b(whats|what is|how is) the (status|project)\b/.test(normalized)) {
    return replyPlan(renderStrictNoAiProjectStatusReply(plannerContext), 0.98, "Answered project status from shell context.", {
      inputText: text,
      plannerContext
    });
  }
  ...
  if (normalized.startsWith("finalize") || normalized.startsWith("resolve")) {
    ...
    return replyPlan(renderStrictNoAiReply(), 0.98, "No-AI mode requires an explicit ticket id for resolve/finalize.");
  }

  if (/\b(prioritize|what is needed|resolve what is needed|achieve that goal|for beta(?:-testing)?|beta readiness)\b/.test(normalized)) {
    return replyPlan(renderStrictNoAiReply(), 0.98, "No-AI mode rejected broad goal-driven mutation prose.");
  }
...
function renderStrictNoAiProjectStatusReply(plannerContext = {}) {
  const summary = plannerContext?.summary ?? {};
  const projectName = path.basename(plannerContext?.root ?? process.cwd());
  const rankedActiveTickets = rankStatusTickets(summary.activeTickets ?? []);
  const modules = Array.isArray(summary.modules) ? summary.modules : [];
  const ticketHint = rankedActiveTickets[0]
    ? `Top active ticket: ${rankedActiveTickets[0].id} (${rankedActiveTickets[0].lane}).`
    : "No active ticket is obvious yet.";
  const moduleHint = modules.length
    ? `Main areas: ${modules.slice(0, 5).map((item) => item.name).join(", ")}.`
    : "Module summary is not available yet.";
  const counts = [];
  ...
  if (looksLikeRepoExplainerQuestion(inputText)) {
    ...
    if (moduleMatches.length) {
      sections.push([
        ...
        ...moduleMatches.slice(0, 3).map((item, index) => index === 0
          ? `- ${item.name}${item.responsibility ? `: ${item.responsibility}` : ""}\nResolved target: ${item.name}`
          : `- ${item.name}${item.responsibility ? `: ${item.responsibility}` : ""}`)
      ].join("\n"));
    }
...
function collectShellFileTargets({ plannerContext, statusPayload, searchLines }) {
  ...
  if (!targets.size) {
    targets.add(path.resolve(root, "cli/lib/shell.ts"));
    targets.add(path.resolve(root, "tests/shell.test.ts"));
    targets.add(path.resolve(root, "tests/shell-human-language.test.ts"));
  }
  return [...targets];
}
...
export async function runInteractiveShell(options) {
  const rl = ServiceHub.terminal?.getInterface?.() ?? readline.createInterface({ input, output });
  options.rl = rl;
  options.blacklist = new Set();
  options.history ??= [];
...
function renderVerifiedFixFinalization(result) {
  const lines = [
    ...
    result.verification?.dogfood ? `Dogfood: ${result.verification.dogfood.status}.` : null,
...
function renderGroundedContextualRepoExplainerReply({ inputText, modules = [] }) {
  ...
  const top = moduleMatches[0];
  return `${top.name} is the relevant service here. ${top.responsibility ?? "It is a tracked repo module."}`;
}
...
function renderRefactorSupportAssessmentReply({ summary }) {
  ...
  return [
    ...
    "Patching support exists through core/lib/patch.js plus workflow-native codelets such as aiwf-common-core/shared/codelets/refactor-ticket.json.",
...
import { analyzeCodeletSideEffects, formatSideEffects } from "aiwf-common-core/services/side-effects";
...
export function chooseShellPlannerModel(ollamaProvider) {
  const models = Array.isArray(ollamaProvider?.models)
    ? ollamaProvider.models.map(normalizeShellPlannerCandidate).filter((model) => model.id)
    : [];
  if (!models.length) {
...
function normalizeShellPlannerCandidate(model) {
  const normalized = { ...(model ?? {}) };
  normalized.id = String(normalized.id ?? normalized.name ?? "").trim();
  if (normalized.sizeB == null && Number.isFinite(normalized.size)) {
    normalized.sizeB = normalized.size / (1024 ** 3);
  }
  return normalized;
}
