/**
 * @file doctor.js
 * @brief Auto-generated header for doctor.js. Needs detailed responsibility and scope.
 */
...
import { discoverProviderState, probeOllama, resolveOllamaConfig } from "aiwf-common-core/services/providers";
import { applyModelFitMatrix, buildModelFitMatrix } from "aiwf-common-core/services/model-fit";
import { leanCtxInstallHint, leanCtxSetupHint, probeLeanCtx } from "aiwf-common-core/services/lean-ctx";
...
export function renderDoctorReport(report) {
  ...
  lines.push("ollama expected shell host: http://127.0.0.1:11434");
  if (report.ollama.host !== "http://127.0.0.1:11434") {
    lines.push("ollama setup hint: run `ai-workflow set-ollama-hw --global --host http://127.0.0.1:11434` if `127.0.0.1` is the intended local planner host");
  }
  if (report.ollama.status === "configured-unreachable") {
    lines.push("ollama reachability hint: the host is configured but not responding; verify network reachability to `http://127.0.0.1:11434` and rerun `ai-workflow doctor`");
  }
  if (report.ollama.status === "missing") {
    lines.push("ollama setup hint: local shell planning is expected on `http://127.0.0.1:11434`; configure it with `ai-workflow set-ollama-hw --global --host http://127.0.0.1:11434`");
  }
