...
import { detectExecutionMode } from "aiwf-common-core/services/execution-context";
...
import { handleShell } from "./shell.ts";
...
import { refreshProviderQuotaState } from "aiwf-common-core/services/providers";
import { refreshCodeletRegistry, listCodeletsFromStore, getCodeletFromStore, searchCodeletsFromStore } from "aiwf-common-core/services/codelets";
import { executeCodelet } from "aiwf-common-core/services/codelet-executor";
import { buildTicketEntity, importLegacyProjections, inferTicketLane, renderEpicsProjection, renderKanbanProjection, writeProjectProjections } from "aiwf-common-core/services/projections";
import { addManualNote, createProjectAssessment, createTicket, enrichProjectGuidelines, evaluateProjectReadiness, getEpic, getProjectAssessment, getProjectMetrics, getProjectSummary, listEpicUserStories, listEpics, listProjectAssessments, resolveProjectNote, reviewProjectCandidates, searchEpicUserStories, searchEpics, searchProject, syncProject, updateTicketLifecycle, withWorkflowStore } from "aiwf-common-core/services/sync";
...
import { updateKnowledgeRemote } from "aiwf-common-core/services/knowledge";
import { assertSafeRepairTarget, getToolkitRoot as getOperatingToolkitRoot, resolveOperatingContext } from "aiwf-common-core/lib/operating-context";
import { readLatestRunArtifact } from "aiwf-common-core/lib/run-artifacts";
...
import { invalidateModelFitCache } from "aiwf-common-core/services/model-fit";
import { invalidateWebSearchCache } from "aiwf-common-core/services/web-search";
...
import { STATUS_NODE_TYPES, formatStatusReport, resolveProjectStatus } from "aiwf-common-core/services/status";
import { listWorkflowIssues, refineWorkflowIssue } from "aiwf-common-core/services/workflow-refinement";
...
async function handleRun(rest) {
  ...
  const normalizedResult = parsedArgs.json && typeof result === "string"
  ...
  if (parsedArgs.json) {
    ...
    process.stdout.write(ShellPresenter.formatCodeletResult(codelet.id, normalizedResult));
  }
...
async function handleProject(rest) {
  ...
  if (subcommand === "status") {
    ...
    const report = await ServiceHub.execute("status", {
      selector,
...
async function handleAudit(rest) {
  ...
  if (sub === "workflow") {
    return runNodeScript(
      path.resolve(toolkitRoot, "runtime", "scripts", "ai-workflow", "workflow-audit.ts"),
      rest.slice(1)
    );
  }
