...
if [[ -n "${AI_WORKFLOW_TOOLKIT_ROOT:-}" && -f "${AI_WORKFLOW_TOOLKIT_ROOT}/aiwf-shell/cli/ai-workflow.ts" ]]; then
  exec node "${AI_WORKFLOW_TOOLKIT_ROOT}/aiwf-shell/cli/ai-workflow.ts" "$@"
fi

if [[ -f "$TOOLKIT_ROOT_FILE" ]]; then
  TOOLKIT_ROOT="$(cat "$TOOLKIT_ROOT_FILE")"
  if [[ -f "${TOOLKIT_ROOT}/aiwf-shell/cli/ai-workflow.ts" ]]; then
    exec node "${TOOLKIT_ROOT}/aiwf-shell/cli/ai-workflow.ts" "$@"
  fi
fi
