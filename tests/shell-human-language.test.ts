...
function withContinuationState(options) {
  return {
    ...
    activeGraphState: {
      ...
      lastReply: "Previous answer: inspect cli/lib/shell.js and keep the patch bounded.",
      focus: {
        taskClass: "code-generation",
        subject: "shell planner replies",
  ...
  return {
    ...
    activeGraphState: {
      ...
      intent: {
        ...
        followUpMode: "continue-prior-work",
        references: {
          files: ["cli/lib/shell.ts", "tests/shell.test.ts"],
          modules: ["cli/lib/shell"],
          graphNodeIds: ["n1", "n2"],
  ...
  return {
    ...
    activeGraphState: {
      ...
      intent: {
        ...
        followUpMode: "continue-prior-work",
        references: {
          files: ["cli/lib/shell.ts", "tests/shell-human-language.test.ts"],
          modules: ["cli/lib/shell"],
          graphNodeIds: ["n1"],
          evidence: ["shell continuation work summary"]
        }
      },
      lastReply: "Shell continuation work currently centers on cli/lib/shell.js and tests/shell-human-language.test.js, with the next step being a smaller grounded fix.",
      focus: {
        taskClass: "summarization",
        subject: "shell continuation work",
        searchQuery: "cli/lib/shell",
...
async function createShellHumanFixture() {
  ...
  await writeFile(path.join(root, "cli", "lib", "shell.ts"), "export const surface = 'shell';\n", "utf8");
  await writeFile(path.join(root, "core", "services", "projections.ts"), [
  ...
  await writeFile(path.join(root, "core", "services", "router.ts"), "export function routeTask() { return 'router'; }\n", "utf8");
  await writeFile(path.join(root, "core", "services", "providers.ts"), "export function discoverProviders() { return []; }\n", "utf8");
  await writeFile(path.join(root, "core", "services", "provider-routing.ts"), "export function pickProviderRoute() { return 'providers'; }\n", "utf8");
  await writeFile(path.join(root, "core", "services", "sync.ts"), "export function syncProjectState() { return true; }\n", "utf8");
  await writeFile(path.join(root, "runtime", "scripts", "ai-workflow", "sync.ts"), "export const runtimeSync = true;\n", "utf8");
  ...
  await writeFile(path.join(root, "tests", "shell.fixture.test.ts"), "export const fixture = true;\n", "utf8");
