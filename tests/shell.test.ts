...
test("buildShellPlannerPrompt grounds repo explainer questions with module evidence", async () => {
  ...
  await fs.writeFile(
    path.join(root, "core", "services", "projections.ts"),
    [
...
test("heuristic shell planner returns a graded assessment for codebase evaluation prompts", () => {
  const assessment = planShellRequestHeuristically("what do you think about the codebase?", {
    ...
    knowledge: {
      ...
      facts: ["Verified fix against 1 command.", "Changed files: cli/lib/shell.ts"]
    },
    summary: {
      ...plannerContext.summary,
      activeTickets: [
...
test("heuristic fallback answers refactor-support assessment prompts with concrete gaps", async () => {
  ...
  assert.match(plan.reply ?? "", /core\/db\/sqlite-store\.js/i);
  assert.match(plan.reply ?? "", /core\/lib\/patch\.js/i);
...
test("heuristic continuation can revise prior answers into requested formats", () => {
  const plan = planShellRequestHeuristically("make that answer one sentence.", plannerContext, {
    activeGraphState: {
      ...
      lastReply: "Shell continuation work currently centers on cli/lib/shell.js and tests/shell-human-language.test.js.",
      graph: {
        nodes: [
...
test("planShellRequest prefers the AI planner for conversational follow-ups with prior state", async () => {
  ...
  const options = {
    ...
    history: [],
    activeGraphState: {
      request: "implement a bounded fix in cli/lib/shell.ts",
      active: false,
      intent: {
        capability: "coding",
        taskClass: "code-generation",
