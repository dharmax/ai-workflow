...
test("inferTicketWorkingSet keeps plan-profile working sets implementation-first", async () => {
  ...
  try {
    ...
    assert.equal(result.evidence.some((item) => item.kind === "selected-file" && item.target === "core/services/shell-retrieval.ts"), true);
  } finally {
    await rm(targetRoot, { recursive: true, force: true });
  }
});
...
async function seedImplementationFirstFixture(targetRoot) {
  ...
  await writeFile(path.join(targetRoot, "core", "services", "shell-retrieval.ts"), [
  ...
  await writeFile(path.join(targetRoot, "core", "services", "context-packer.ts"), [
    "export function buildSurgicalContext() {",
    "  return 'context';",
    "}"
  ].join("\n"), "utf8");
  await writeFile(path.join(targetRoot, "tests", "workflow-noise.test.ts"), "export const workflow = 'workflow retrieval workflow context';\n", "utf8");
  await writeFile(path.join(targetRoot, "tests", "context-noise.test.ts"), "export const context = 'context workflow retrieval';\n", "utf8");
  await writeFile(path.join(targetRoot, "tests", "shell-retrieval.test.ts"), "import { inferTicketRetrievalContextFromStore } from '../core/services/shell-retrieval.ts';\nexport const covered = inferTicketRetrievalContextFromStore;\n", "utf8");
  ...
  return withWorkflowStore(targetRoot, async (store) => {
    ...
    store.appendArchitecturalPredicate({
      ...
      objectId: "file:core/services/shell-retrieval.ts"
    });
    store.appendArchitecturalPredicate({
      subjectId: entity.id,
      predicate: "relates_to",
      objectId: "file:core/services/context-packer.ts"
    });
    store.appendArchitecturalPredicate({
      subjectId: entity.id,
      predicate: "validated_by",
      objectId: "test:tests/shell-retrieval.test.ts"
    });
    if (retrievalSymbol) {
      store.appendArchitecturalPredicate({
        subjectId: entity.id,
...
async function seedWeakEvidenceFixture(targetRoot) {
  ...
  await writeFile(path.join(targetRoot, "tests", "workflow-retrieval-noise.test.ts"), "export const workflow = 'workflow retrieval playbook context';\n", "utf8");
