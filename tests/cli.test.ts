...
test("shell creates a Telegram remote-control epic end to end in mutating mode", async () => {
  ...
  const preloadPath = path.join(projectRoot, "shell-preload.ts");
...
test("one-shot shell handles project-status questions locally without planner fetches", async () => {
  ...
  const preloadPath = path.join(projectRoot, "shell-status-preload.ts");
...
test("one-shot AI shell requests report non-interactive progress and selected model", async () => {
  const projectRoot = await makeTempDir();
  const preloadPath = path.join(projectRoot, "shell-ai-preload.ts");
...
test("generated helper scripts work against initialized project state", async () => {
  ...
  try {
    ...
    const workflowAuditInitial = await runNode(["scripts/ai-workflow/workflow-audit.ts"], { cwd: targetRoot });
    ...
    const ticketResult = await runNode(
      ["scripts/ai-workflow/kanban-ticket.ts", "--id", "TKT-001"],
      { cwd: targetRoot }
    );
    ...
    const guidanceResult = await runNode(
      ["scripts/ai-workflow/guidance-summary.ts", "--ticket", "TKT-001", "--changed"],
      { cwd: targetRoot }
    );
    ...
    const reviewResult = await runNode(["scripts/ai-workflow/review-summary.ts"], { cwd: targetRoot });
    ...
    const verifyResult = await runNode(
      ["scripts/ai-workflow/verification-summary.ts", "--cmd", "node -e \"console.log('ok')\""],
      { cwd: targetRoot }
    );
    ...
    const guidelineAuditFail = await runNode(["scripts/ai-workflow/guideline-audit.ts"], { cwd: targetRoot });
...
test("dogfood report is regenerated through the runtime script and audit fails when the report is missing or stale", async () => {
  ...
  try {
    ...
    const missingAudit = await runNode(["scripts/ai-workflow/workflow-audit.ts", "--json"], { cwd: targetRoot });
    ...
    const dogfoodResult = await runNode(
      ["scripts/ai-workflow/dogfood.ts", "--surface", "shell,provider,workflow,init", "--profile", "bootstrap", "--json"],
      { cwd: targetRoot }
    );
    assert.equal(dogfoodResult.code, 0, dogfoodResult.stderr || dogfoodResult.stdout);

    const scriptPath = path.join(targetRoot, "scripts", "ai-workflow", "workflow-audit.ts");
    const currentAuditScript = await readFile(scriptPath, "utf8");
    await writeFile(scriptPath, `${currentAuditScript}\n// stale dogfood check\n`, "utf8");

    const staleAudit = await runNode(["scripts/ai-workflow/workflow-audit.ts", "--json"], { cwd: targetRoot });
...
test("dogfood full shell profile uses the local Ollama path for the soft shell scenario", async () => {
  const targetRoot = await createShellFixtureProject();
  const preloadPath = path.join(targetRoot, "dogfood-ollama-preload.ts");
