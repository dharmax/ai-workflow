...
export class ShellAgent {
  ...
  async turn(
    ...
    for (let stepIndex = 1; stepIndex <= maxSteps; stepIndex++) {
      ...
      try {
        ...
        if (res.ok && res.text) {
          ...
          const timeoutDetail = res.failure.kind === 'timeout'
            ? ` (timed out after ${turnTimeout}ms - model may be cold-loading or busy)`
            : '';
          const failureMsg = `Model communication issue: ${res.failure.message || res.failure.kind}${timeoutDetail}`;
          stepTrace.thought = failureMsg;
          steps.push(stepTrace);

          const output = allObservations.length > 0
          ...
          const emptyMsg = 'No response generated by the model.';
    ...
    // If max steps reached (model kept calling tools on all steps), ask for final summary
    let finalOutput = '';
    try {
      ...
      if (summaryRes.ok && summaryRes.text) {
        finalOutput = summaryRes.text.trim();
      }
    } catch {
