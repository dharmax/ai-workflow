...
- `node aiwf-shell/scripts/ai-workflow/kanban.mjs new` can succeed even when the post-write assessment hook emits `Unexpected end of JSON input`. That failure is worth hardening because it obscures whether the write or the follow-up assessment actually failed.
