...
import { CompletionEngine, GoogleAdapter, OpenAIAdapter, AnthropicAdapter, OllamaProvider } from "@dharmax/llm-utils";
import { openWorkflowStore } from "../db/sqlite-store.mjs";
import { sha1 } from "../lib/hash.mjs";
import { getGlobalConfigPath, getProjectConfigPath, isConfigWriteAccessError, readConfig, readConfigSafe, writeConfigValue } from "../../cli/lib/config-store.mjs";
...
function normalizeConfiguredModels(providerId, config = {}, fallback = []) {
  ...
  return source.map((entry) => {
    ...
    const profile = profileModel(model.id, model.sizeB, []);
    ...
    return {
      ...
      capabilities: profile.capabilities,
      ...model,
      quality: model.quality ?? (mockProvider ? "high" : profile.quality),
      capabilities: model.capabilities ?? (mockProvider ? { logic: 5, strategy: 5, prose: 5, creative: 3, visual: 5, data: 5 } : profile.capabilities),
...
function profileModel(id, sizeB = null, reference = []) {
  ...
  if (ref) {
    return {
      ...
      capabilities: ref.capabilities ?? inferCapabilities(lower),
      strengths: ref.strengths ?? []
    };
  }
  const numericSize = normalizeModelSize(sizeB) ?? estimateOllamaModelSizeB(id);
  const isMockOrJudge = /^mock(?:[-_:]|$)/.test(lower) || /judge|smart|brain|planner/.test(lower);
  return {
    quality: isMockOrJudge || numericSize >= 20 || /gpt-4|claude|opus|sonnet|pro/.test(lower) ? "high" : "medium",
    capabilities: isMockOrJudge ? { logic: 5, strategy: 5, prose: 5, creative: 3, visual: 5, data: 5 } : inferCapabilities(lower),
    strengths: []
  };
}

function inferCapabilities(lower) {
  const base = { logic: 3, strategy: 3, prose: 3, creative: 2, visual: 2, data: 3 };
  if (/coder|code|gpt-4|claude|sonnet|pro/.test(lower)) {
    base.logic = 5;
    base.strategy = 5;
    base.data = 4;
  }
  if (/judge|smart|brain|planner|reason/.test(lower)) {
    base.logic = Math.max(base.logic, 4);
    base.strategy = Math.max(base.strategy, 4);
    base.prose = Math.max(base.prose, 4);
  }
  if (/flash|haiku|mini/.test(lower)) {
    base.data = Math.max(base.data, 4);
  }
  return base;
}
