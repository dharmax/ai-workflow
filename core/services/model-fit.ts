/**
 * @file model-fit.js
 * @brief Auto-generated header for model-fit.js. Needs detailed responsibility and scope.
 */

import path from "node:path";
import { mkdir, readFile, rm, writeFile } from "node:fs/promises";
import { sha1 } from "../lib/hash.ts";
import { searchWebEvidence } from "./web-search.ts";
...
async function enrichMatrixWithRemoteEvaluator({ providerState, taskClass, evaluator, heuristics, webEvidence }) {
  ...
  try {
    const { generateCompletion } = await import("./providers.ts");
