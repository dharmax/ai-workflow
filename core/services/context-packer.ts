...
export async function buildSurgicalContext(projectRoot, { symbolNames = [], filePaths = [], ticketId = null } = {}) {
  ...
  return withWorkflowStore(projectRoot, async (store) => {
    ...
    // Properly utilize HeuristicContextManager for gathering and compression
    const manager = new HeuristicContextManager({
      async query(query, categories) {
        const results: any[] = [];
        // Handle files if query looks like a path or if we want to force inclusion
        const targetFiles = categories.includes("file") ? [query] : [];
        for (const path of targetFiles) {
          try {
            const file = await readProjectFile(projectRoot, path);
            results.push({ id: path, category: "file", title: path, body: file.content, tags: [] });
          } catch { /* skip */ }
        }
        return results;
      },
