/**
 * @file workspace-mutation.js
 * @brief Auto-generated header for workspace-mutation.js. Needs detailed responsibility and scope.
 */
...
export async function withWorkspaceMutation(root, operation, callback, { writeProjections = true, syncAfter = true, syncBefore = true } = {}) {
  ...
  try {
    ...
    if (syncBefore && before.dirty) {
      const { syncProject } = await import("../services/sync.ts");
    ...
    try {
      ...
      const { collectProjectFileSnapshot } = await import("./filesystem.ts");
...
async function recordWorkspaceMutation(root, mutation) {
  const { openWorkflowStore } = await import("../db/sqlite-store.ts");
