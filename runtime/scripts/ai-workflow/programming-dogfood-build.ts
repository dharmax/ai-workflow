...
const MODEL_JS = `import { CONFIG } from "./config.ts";
...
const UPDATE_JS = `import { CONFIG } from "./config.ts";
import { cloneState, createEnemyWave, createInitialState } from "./model.ts";
...
const MAIN_JS = `import { createInitialState } from "./game/model.ts";
import { updateGame } from "./game/update.ts";
import { renderScene } from "./game/render.ts";
import { renderGameToText } from "./game/text-state.ts";
...
import { updateGame } from "../src/game/update.ts";
...
const TEMPLATE_FILES = {
  ...
  "scripts/dev-server.ts": DEV_SERVER_MJS,
  "index.html": INDEX_HTML,
  "styles.css": STYLES_CSS,
  "src/game/config.ts": CONFIG_JS,
  "src/game/model.ts": MODEL_JS,
  "src/game/update.ts": UPDATE_JS,
  "src/game/render.ts": RENDER_JS,
  "src/game/text-state.ts": TEXT_STATE_JS,
  "src/main.ts": MAIN_JS,
  "tests/game-logic.test.ts": TEST_JS,
...
async function runNode(args, cwd) {
  try {
    const { stdout, stderr } = await execFileAsync( "npx", "tsx", args, { cwd, maxBuffer: 8 * 1024 * 1024 });
...
export async function runProgrammingDogfood(argv = process.argv.slice(2)) {
  ...
  if (!skipInit) {
    await runNode([path.join(REPO_ROOT, "scripts", "init-project.ts"), "--target", targetRoot], REPO_ROOT);
  }

  await writeTemplateFiles(targetRoot);
  await mergePackageJson(targetRoot);

  const testResult = await runNode(["--test", "tests/game-logic.test.ts"], targetRoot);
